...
/** Consulta o status na uazapi e sincroniza a coluna status do banco. */
Deno.serve(async (req) => {
  ...
  const state = (payload?.status ?? {}) as Record<string, unknown>
  const jid = (state?.jid ?? null) as Record<string, unknown> | null
  const status = normalizeStatus(instance?.status) ?? (state?.connected ? 'connected' : 'disconnected')

  const updates: Record<string, unknown> = { status }
  if (typeof instance?.id === 'string') updates.instance_id = instance.id
  ...
  return jsonResponse({
    ...
    number: jid && typeof jid.user === 'string' ? jid.user : null,
    ...
    lastConnectedAt: updated?.last_connected_at ?? null,
  })
})
